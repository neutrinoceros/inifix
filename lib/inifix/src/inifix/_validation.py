...
def collect_exceptions_for_elementary_item(
    ...
    if not isinstance(key, str):
        exceptions.append(
            ValueError(f"Found key '{key}' with type {type(key)}. Expected a str")
        )
        return exceptions
    ...
    if len(invalid_values) == 1:
        exceptions.append(
            ValueError(
                f"Key {key!r} is associated to value {value} with type {type(value)}, "
        ...
        exceptions.append(
            ValueError(
                ...
                f"{[type(v) for v in invalid_values]}, respectively. "  # pyright: ignore[reportUnknownArgumentType]
        ...
        if isinstance(v, dict):
            ...
            match sections_mode:
                case SectionsMode.ALLOW | SectionsMode.FORBID:
                    section_exceptions: list[ValueError] = (
                        collect_exceptions_for_elementary_item(k, v)
                    )
                    if section_exceptions:
                        section_to_exceptions[k] = section_exceptions
